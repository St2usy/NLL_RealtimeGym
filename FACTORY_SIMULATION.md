...
## ✅ 업데이트: 라인 분리 및 실제 로봇 운반 시스템

**2025-12-01 업데이트:**
- ✅ **라인 완전 분리**: Line 1과 Line 2가 독립적으로 작동, 아이템과 로봇이 라인을 넘지 않음
- ✅ **정확한 설비 구성**: 각 라인당 Storage(1), Washer(1), Cutter(2), Cooker(2), Plating(1), Sealing(1), VisionQA(1), Storage(1)
- ✅ **실제 로봇 운반**: Logistic Robot이 스테이션 간 아이템을 실제로 픽업하고 운반함 (텔레포트 제거)
- ✅ **라인 구분선**: 화면 중앙에 LINE 1과 LINE 2를 구분하는 굵은 세로선 표시
